...
import { getApprovedAllDataAff } from '../../store/action/action';
import config from '../../Config/Config';


class AffiliatApproveList extends Component {
	constructor(props){
		super(props);
	...
	approve(e){
		fetch(`${config.base_url}/affiliat_approved_status`,{
			method: 'POST',
			headers: new Headers({
									...
									<Table striped bordered hover size="sm">
										...
										{ this.props.state_aff_approveds_all === undefined || this.props.state_aff_approveds_all.length === 0 ? null : 
											...
											<tr key={i}>
												...
												<td>{userrequest.status === 0 ? 'Panding' : userrequest.status === 1 ? 'Approved' : userrequest.status === 2 ? 'Disapproved' : null}</td>
...
export default connect(mapStateToProps, {getApprovedAllDataAff}) (AffiliatApproveList);
