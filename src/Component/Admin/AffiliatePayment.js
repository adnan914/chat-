...
import { getAllPurchasedPrograms, getUserByUserRoll } from '../../store/action/action';
import config from '../../Config/Config';


class AffiliatePayment extends Component {
	constructor(props){
		super(props);
	...
	async componentDidMount(){
		await this.props.getUserByUserRoll('Affiliate')
	...
	timer(){
		if (this.props.state_get_all_purchased_programs === undefined ) {	
			...
			for (let i=0; proLength.length > i; i++) {
				if (proLength[i].affiliate_id === this.state.affiliate_id) {
	...
	submit(){
		if (this.state.array.length === 0) {
			...
			fetch(`${config.base_url}/purchased_programs_multiple_update_affiliate`,{
				method: 'POST',
				headers: new Headers({
							...
							<Table striped bordered hover size="sm">
								...
								<tbody>
									{this.state.program.length === 0 ? <tr><td colSpan="5">No Payment</td></tr> : this.state.program.map((pro, i) => (
										<tr key={i}>
											<td>{pro.affiliate_part.status === 0 ? <input type="checkbox" name={i} value={pro._id} onChange={this.checkHanler} /> : <span className="checked"><i className="fa fa-check-square-o"></i></span>}</td>
											<td>{i+1}</td>
											<td>Rs.{pro.course_name}</td>
											<td>Rs.{pro.affiliate_part.ammount}</td>
											<td>{pro.affiliate_part.status === 0 ? 'Panding' : 'Compeleted'}</td>
										</tr>
									))}
								</tbody>
							</Table>
...
export default connect(mapStateToProps, {getAllPurchasedPrograms, getUserByUserRoll}) (AffiliatePayment);